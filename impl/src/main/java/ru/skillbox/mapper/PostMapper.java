...
public interface PostMapper {
    ...
    default Integer mapCommentsCount(List<PostComment> postCommentList) {
        if (postCommentList != null) {
            ...
            return (int) postCommentList.stream()
                    ...
                    .filter(p -> p.getParentId() == 0L || p.getParentId() != 0L
                            && postCommentMap.get(p.getParentId()).getIsDelete().equals(false)).count();
        }
        return 0;
    }
    ...
    default Integer mapLikeAmount(List<PostLike> likes) {
        ...
        return (int) likes.stream().filter(p -> p.getIsDelete().equals(false))
                .filter(p -> p.getPerson().getIsEnabled().equals(true)).count();
    }
